    ...
    for tid, detections in tracks_dict.items():
        ...
        for det in detections[1:]:
            ...
            size_ok = (1/max_size_ratio) <= w_ratio <= max_size_ratio and \
                      (1/max_size_ratio) <= h_ratio <= max_size_ratio

            need_split = (dt > max_dt) or (center_jump > max_center_jump) or (not size_ok)
...
def get_ptracks(video_path, tracker_model, plot=False):
    ...
    while cap.isOpened():
        ...
        if success:
            ...
            result = tracker_model.track(frame, persist=True, tracker='botsort.yaml',verbose=False)[0]
...
for video_path in sys.argv[1:]:
    ...
    for i,p in enumerate(persons):
        ...
        motion, motion_total, frames_used, frames_with_hands = measure_hand_motion(video_path=outfile, model=pose_model,stride=30)
